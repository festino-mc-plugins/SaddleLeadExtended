...
public class UtilsWorld
{
	...
	// https://github.com/Bukkit/mc-dev/blob/master/net/minecraft/server/EntityLiving.java#L1067
	public static final double LINEAR_DRAG_H = 0.91;
	// https://github.com/Bukkit/mc-dev/blob/master/net/minecraft/server/EntityLiving.java#L1118
	public static final double LINEAR_DRAG_V = 0.98;
	// https://github.com/Bukkit/mc-dev/blob/master/net/minecraft/server/EntityLiving.java#L1115
	public static final double ACCELERATION_V = -0.08;
	...
	public static Vector nextTickVelocity(Vector v)
	{
		...
		new_v.setX(LINEAR_DRAG_H * v.getX());
		new_v.setY(LINEAR_DRAG_V * (v.getY() - ACCELERATION_V));
		new_v.setZ(LINEAR_DRAG_H * v.getZ());
		return new_v;
	}
	...
	public static Vector throwVector(Location from, Location to, double power, boolean choose_shorter)
    {
        ...
        double v0 = power;
        double min_vh = dh * (1 - LINEAR_DRAG_H);
    ...
    {
		double pow = Math.pow(LINEAR_DRAG_V, n), k = (1 - pow) / (1 - LINEAR_DRAG_V);
    	return k * v0_y + ACCELERATION_V * LINEAR_DRAG_V / (1 - LINEAR_DRAG_V) * (n - k);
    }
	private static double n(double dh, double vh_0)
    {
		double t = 1 - (1 - LINEAR_DRAG_H) * dh / vh_0;
    	return Math.log(t) / Math.log(LINEAR_DRAG_H);
    }
